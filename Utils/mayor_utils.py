...
def get_mayor_perks():
    ...
    for entry in data:
        ...
        start_date = datetime.strptime(temp_start_date, "%Y-%m-%d").replace(
            tzinfo=timezone.utc
        )
    ...
    for mayor in mayors:
        ...
        start_date = datetime.strptime(start_match.group(1), "%m/%d/%Y").replace(
            tzinfo=timezone.utc
        )
...
def match_mayor_perks(timestamp, mayor_data):
    ...
    for mayor in mayor_data:
        if isinstance(mayor["start_date"], str):
            try:
                mayor["start_date"] = datetime.fromisoformat(
                    mayor["start_date"].replace("Z", "+00:00")
                )
            except Exception as e:
    ...
    for i, mayor in enumerate(mayor_data):
        next_start = (
            mayor_data[i + 1]["start_date"] if i + 1 < len(mayor_data) else None
        )
        if next_start:
