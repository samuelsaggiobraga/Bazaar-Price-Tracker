...
for item_id in item_ids:
    ...
    if count == 0:
        continue
    prices = [
        entry["buy"] for entry in data if isinstance(entry, dict) and "buy" in entry
    ]
    if not prices:
        continue
    ...
    if median_price > 100000 and Average_Volume > 5000:
        items.append({"item_id": item_id, "volume_price_weight": volume_price_weight})
